...
App.controller('interceptAccountController', [ '$scope', '$state', '$http',
    '$stateParams', '$location', '$sce', '$q', '$window',
    function($scope, $state, $http, $stateParams, $location, $sce, $q, $window) {
        ...
        $scope.getHttpMetrics = function () {
            ...
            $http.post(url, params).success(function (data) {
                ...
                data.main.tooltip.formatter = function(params,ticket) {
                    var content = '';
                    for(var i=0;i<params.length;i++) {
                        var param = params[i];
                        if(param.data.main!=0)
                ...
                data.main1.tooltip.formatter = function(params,ticket) {
                    var content = '';
                    for(var i=0;i<params.length;i++) {
                        var param = params[i];
                        if(param.data.main1!=0)
                            content += "</li>"+param.seriesName+" : "+param.data+"</li></br>";
                    }
                    return '<div style="text-align:left">'+'<p style="text-align:center">'+params[0].name+'</p>'+content+'</div>'
                }
                data.main2.tooltip.formatter = function(params,ticket) {
                    var content = '';
                    for(var i=0;i<params.length;i++) {
                        var param = params[i];
                        if(param.data.main2!=0)
                            content += "</li>"+param.seriesName+" : "+param.data+"</li></br>";
                    }
                    return '<div style="text-align:left">'+'<p style="text-align:center">'+params[0].name+'</p>'+content+'</div>'
                }
                $scope.timeTitle = data.main.title.text;
                $scope.timeTitle1 = data.main1.title.text;
                $scope.timeTitle2 = data.main2.title.text;
                myLineChart.setOption(data.main, true);
                myLineChart1.setOption(data.main1, true);
                myLineChart2.setOption(data.main2, true);
                $scope.loading = false;
            }).error(function () {
